...
class forest{
  public:
    forest();
    forest(char* , const double);
    void randomForest();
    void decisionTree(std::vector< std::vector<double> >& , const int go = 0);
    void randomForest(std::vector< std::vector<double> >& );
    bool continu(const std::vector< std::vector<double> >&);
  private:
    double eps;
    size_t trees;
...
bool forest::continu(const vector< vector<double> >& input){
...
forest::forest( char* Filename, const double trees_){
  eps = 0;
  trees = trees_;
  int total = 0;
...
void forest::decisionTree(vector< vector<double> >& input, const int go){
  if(go == 1)
  ...
  if( confusion <= eps || !(continu(input))){
   ...
   total_con = ( ( YLeft + NLeft) / (YLeft +NLeft + (Y - YLeft) + (N - NLeft))) * ( 1.0-pow(YLeft/(YLeft+NLeft),2)-pow(NLeft/(YLeft+NLeft),2) ) + (((Y - YLeft) + (N - NLeft) )/(YLeft + NLeft +(Y - YLeft) + (N - NLeft))) * ( 1.0-pow( (Y - YLeft) / ( (Y - YLeft)+ (N - NLeft)),2)-pow( (N - NLeft) /( (Y - YLeft) + (N - NLeft)),2));
    ...
    for(int k = 0; k < Store_list.size(); k++){
      if(Store_list[k][0] < minConf){
        minConf = Store_list[k][0];
        headcol = k;
      }
    }
...
void forest::randomForest(){
  randomForest(variables);
}

void forest::randomForest(vector< vector<double> >& Data){
  FILE *fout = fopen("forest_pred_func.cpp", "w");
  fprintf(fout, "int forest_predict(double *attr){\n");
  fprintf(fout, "int Y = 0;\n");
  fprintf(fout, "int N = 0;\n");
  srand(unsigned(time(0)));
  size_t size = variables.size();
  vector< vector<double> > subInput;
  vector<size_t> positioin(size);
  for(int i = 0 ; i < size; i++)
    positioin[i] = i;
  for(int i = 0; i < trees; i++){
    fprintf(fout, "tree%ld_predict:\n", i+1);
    random_shuffle(positioin.begin(), positioin.end());
    for(int j = 0; j < (size / 3); j++){
      subInput.push_back(variables[positioin[j]]);
    }
    decisionTree(subInput);
...
int main(int argc, char** argv){
  ...
  size_t trees = atol(argv[2]);
  forest Run(argv[1], trees);
  Run.randomForest();
